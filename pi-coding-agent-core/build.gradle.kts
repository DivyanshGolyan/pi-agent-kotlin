...
description = "Kotlin port of the pi-mono coding-agent session and message slice."

mavenPublishing {
    coordinates(providers.gradleProperty("GROUP").get(), "pi-coding-agent-core", providers.gradleProperty("VERSION_NAME").get())
...
tasks.register<Test>("parityTest") {
    group = "verification"
    description = "Runs pi-coding-agent-core parity tests."
    testClassesDirs =
        sourceSets.test
            .get()
            .output.classesDirs
